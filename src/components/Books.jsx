    ...
    >
      ...
      <div style={{ display:'flex', gap:'10px', marginBottom:'20px', flexWrap:'wrap' }}>
        ...
        <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}
          style={{ background:'var(--bg-input)', border:'1px solid var(--border)', borderRadius:'8px', padding:'10px 14px', color: selectedCategory ? 'var(--text-1)' : 'var(--text-3)', fontFamily:"'Manrope',sans-serif", fontSize:'14px', outline:'none', minWidth:'150px' }}>
          <option value="">All Categories</option>
          {categories.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={sortOrder} onChange={e => setSortOrder(e.target.value)}
          style={{ background:'var(--bg-input)', border:'1px solid var(--border)', borderRadius:'8px', padding:'10px 14px', color:'var(--text-1)', fontFamily:"'Manrope',sans-serif", fontSize:'14px', outline:'none' }}>
          ...
          <div style={{ background:'var(--bg-2)', border:'1px solid var(--border)', borderRadius:'16px', maxWidth:'520px', width:'100%', maxHeight:'90vh', overflowY:'auto', position:'relative', boxShadow:'0 20px 60px rgba(0,0,0,0.40)' }}
            ...
            <div style={{ padding:'24px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'6px' }}>
                <h2 style={{ fontSize:'22px', fontWeight:'700', color:'var(--text-1)', fontFamily:"'Bodoni Moda',serif", lineHeight:1.25, flex:1, paddingRight:'12px' }}>{selectedBook.title}</h2>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'10px', background:'#fef3c7', border:'1px solid #fde68a', borderRadius:'10px', padding:'12px 16px', marginBottom:'16px' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#96702E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
                </svg>
                <div>
                  <p style={{ fontSize:'11px', color:'#96702E', fontWeight:'700', letterSpacing:'0.05em', textTransform:'uppercase' }}>Shelf Location</p>
